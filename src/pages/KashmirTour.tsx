...
const getActivityIconAndText = (text: string) => {
  ...
  switch (matchedEmoji) {
    ...
    case "🌷":
    case "🍎":
      return { icon: Sparkles, iconColor: "text-primary", text: cleanText };
    case "❄️":
    case "🧥":
    case "🚡":
      return { icon: Snowflake, iconColor: "text-primary", text: cleanText };
    case "🏔️":
    case "🌊":
    case "📸":
      return { icon: Mountain, iconColor: "text-primary", text: cleanText };
    case "📍":
...
const KashmirTour = () => {
  const pkg = packagesData.find(p => p.slug === "kashmir-tour-7-days")!;
